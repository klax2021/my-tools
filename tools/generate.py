...
def classify_against_white(domain, is_wildcard, white_exact, white_wildcard):
    ...
    if not is_wildcard:
        ...
        if any(is_covered_by_wildcard(w, domain) for w in white_exact | white_wildcard):
            return "review"
        return None
    else:
        # 黑名单是泛域名（拦截自身+所有子域）：
...
def clean_blacklist(black_lines, white_exact, white_wildcard):
    ...
    for line in black_lines:
        ...
        result = classify_against_white(domain, is_wildcard, white_exact, white_wildcard)
        if result == "auto":
...
def write_payload_yaml(path, lines):
...
def main():
    ...
    print(f"白名单解析完成：精确域名 {len(white_exact)} 条，泛域名 {len(white_wildcard)} 条")

    # Full / Lite 清洗（域名层级关系判断，而非单纯字符串相等）
    clean_full, removed_full, review_full = clean_blacklist(black_full, white_exact, white_wildcard)
    clean_lite, removed_lite, review_lite = clean_blacklist(black_lite, white_exact, white_wildcard)

    # 写入 Full / Lite
    full_yaml = os.path.join(RULE_DIR, "adblock-clean-full.yaml")
    write_payload_yaml(full_yaml, clean_full)

    lite_yaml = os.path.join(RULE_DIR, "adblock-clean-lite.yaml")
    write_payload_yaml(lite_yaml, clean_lite)
    ...
    print(f"广告规则生成完成（自动剔除 {len(all_removed)} 条，待人工复核 {len(all_review)} 条）")
    ...
    stats = {
        ...
        "review_full_count": len(review_full),
        "review_lite_count": len(review_lite),
        "review_unique_count": len(all_review),
    }
